...
import numpy as np
import sys
def tra_with_padding(input_image, padding_size):

    padded_image = np.pad(input_image, ((padding_size, padding_size), (padding_size, padding_size)), mode='constant')
...
def tra_padding_height(input_image, padding_size):


    # 使用np.pad()函数在输出图像周围添加padding
    padded_image = np.pad(input_image, ((padding_size, padding_size), (padding_size, padding_size)), mode='constant', constant_values=-2)

    return padded_image

def tra_padding_tra(input_image, padding_size):
...
# conf_threshold = 0.5
# 局部图的尺寸 8m = 0.04 *100*2
local_size_half = 100
# max_hm_z = float(world_name.split('_')[-5])
# max_distance = float(world_name.split('_')[-1])
max_hm_z = float(world_name.split('_')[-5])/100
max_distance = float(world_name.split('_')[-1])/100
...
while not rospy.is_shutdown():
    odom_data = rospy.wait_for_message('/wamv/robot_localization/odometry/filtered', Odometry, timeout=None)
    # print(odom_data)

    robot_center_x = int((odom_data.pose.pose.position.x+max_distance/2)/max_distance*height_data.shape[1])
    robot_center_y = int((-odom_data.pose.pose.position.y+max_distance/2)/max_distance*height_data.shape[0])
    height_data_padded = tra_padding_height(height_data, padding_size)
    local_x1 = padding_size+robot_center_x - local_size_half
    local_x2 = padding_size+robot_center_x +  local_size_half
    local_y1 = padding_size+robot_center_y -  local_size_half
    local_y2 = padding_size+robot_center_y +  local_size_half
    ...
    height_data_local = height_data_padded[local_y1:local_y2,local_x1:local_x2]
    height_data_local = cv2.flip(height_data_local, 1)
    ...
    grid_map.info.pose.position.x = odom_data.pose.pose.position.x
    grid_map.info.pose.position.y = odom_data.pose.pose.position.y
    grid_map.info.pose.position.z = 0
    grid_map.info.pose.orientation.x = 0
    grid_map.info.pose.orientation.y = 0
    grid_map.info.pose.orientation.z = 0
    ...
    for key,value in pub_tra_dict_sum_items :
        value_padded = tra_padding_tra(value, padding_size)
        value_local = value_padded[local_y1:local_y2,local_x1:local_x2]
        value_local = cv2.flip(value_local, 1)
        ...
        mat.layout.dim[0].label = "column_index"
        mat.layout.dim[1].label = "row_index"
        mat.layout.dim[0].size = value_local.shape[0]
        mat.layout.dim[1].size = value_local.shape[1]
        mat.layout.dim[0].stride = value_local.shape[1] * value_local.shape[0]
        mat.layout.dim[1].stride = value_local.shape[1]
        mat.layout.data_offset = 0
        mat.data = value_local.flatten()
